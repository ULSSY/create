...
# /20200711/efficientdet_d0_coco17_tpu-32.tar.gz#모델명
#/20200711/efficientdet_d0_coco17_tpu-32.tar.gz
# Download and extract model
def download_model(model_name, model_date):#download_model이라는 함수만들기
...
def save_inference(detection_model,image_np,video_write):
    # The input needs to be a tensor, convert it using `tf.convert_to_tensor`.
    input_tensor = tf.convert_to_tensor(image_np)
    # The model expects a batch of images, so add an axis with `tf.newaxis`.
    input_tensor = input_tensor[tf.newaxis, ...]
    ...
    viz_utils.visualize_boxes_and_labels_on_image_array(
          ...
          min_score_thresh=.30,
          agnostic_mode=False)
    video_write.write(image_np_with_detections)
...
if cap.isOpened() == False :
    ...
    frame_heignt=int(cap.get(4))
    out=cv2.VideoWriter('data/output.avi',cv2.VideoWriter_fourcc('M','J','P','G'),20,(frame_width,frame_heignt))
